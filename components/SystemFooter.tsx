    ...
    return (
        <div 
            className={`
                ...
                ${isMinimized ? 'w-auto px-8 py-3 rounded-full bg-black/90 border-white/20 translate-y-0' : 'w-full rounded-xl bg-[#080808]/90 border-white/10 p-3 md:p-4'}
                border backdrop-blur-3xl shadow-[0_0_80px_rgba(0,0,0,0.8)] border-glow-rose relative group overflow-hidden mx-auto
            `}
        >
            {/* Toggle Handle */}
            <button 
                onClick={() => setIsMinimized(!isMinimized)}
                className={`absolute top-1/2 -translate-y-1/2 text-slate-500 hover:text-pearl transition-colors z-20 p-2 rounded-full hover:bg-white/5 ${isMinimized ? 'right-3' : 'right-2 top-3 -translate-y-0'}`}
                ...
                <div className="flex items-center gap-6 pr-8 animate-fade-in">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[7px] font-mono text-slate-500 uppercase tracking-widest leading-none mb-0.5">Governance_Axiom</span>
                            <span className="font-orbitron text-[10px] text-pearl font-bold uppercase tracking-wider leading-none">{currentMode}</span>
                        </div>
                    </div>
                    <div className="w-px h-4 bg-white/10" />
                    <span className="text-[8px] font-mono text-gold uppercase tracking-[0.2em] opacity-60">NEXUS_MINIMIZED</span>
                </div>
            ) : (
                // Expanded View
                <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0 animate-fade-in pr-6">
                    <div className="flex items-center gap-6 w-full md:w-auto overflow-x-auto scrollbar-none pb-1 md:pb-0">
                        ...
                        <div className="hidden xl:flex items-center gap-6 shrink-0">
                            <span className="text-[8px] font-mono text-slate-500 uppercase tracking-[0.4em] font-black whitespace-nowrap">Control_Nexus_Active</span>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end shrink-0">
                        {SYSTEM_NODES.filter(n => n.isShield || n.isLogs || n.isBridge || n.isAudit).map(node => (
                            <button 
                                ...
                                className={`px-4 py-1.5 border font-orbitron text-[8px] uppercase tracking-[0.2em] rounded-sm transition-all font-black active:scale-95 shadow-md hover:scale-105 ${
                        ...
                        {onOpenConfig && (
                            <button
                                ...
                                className="px-4 py-1.5 border border-slate-700/50 bg-slate-900/20 text-slate-400 font-orbitron text-[8px] uppercase tracking-[0.2em] rounded-sm hover:bg-slate-800 hover:text-pearl transition-all font-black hover:border-pearl/30"
                            >
                                CONFIG
                            </button>
                        )}
